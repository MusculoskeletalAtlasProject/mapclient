...
class BaseSpringLayout(ABC):
    ...
    def __init__(self, dataset=None, nodes: List[Node] = None,
        ...
        self.nodes: List[Node] = _build_nodes(dataset) if nodes is None else nodes
        self._node_identifiers = [n.getIdentifier() for n in (dataset['vertices'] if nodes is None else nodes)]
    ...
    def positions(self):
        return {self._node_identifiers[i]: n.datapoint for i, n in enumerate(self.nodes)}
